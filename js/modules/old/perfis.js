...
// ================================================================
// Versão: 2.5 - CORRIGIDO: Força permissões para Master/Admin
// ================================================================
...
GR.Modules.Perfis = {
    ...
    // ================================================================
    // 🆕 CONTROLE PARA EVITAR LOOP DE FILTRAGEM
    // ================================================================
    ...
    // ================================================================
    // 🆕 FORÇAR CARREGAMENTO DO PERFIL (CORREÇÃO PRINCIPAL)
    // ================================================================
    _forcarCarregamentoPerfil: function() {
        try {
            // 1. VERIFICA SE É MASTER PELO EMAIL (FALLBACK MASTER)
            ...
            if (user && user.email === 'roqueep@gmail.com') {
                ...
                try {
                    localStorage.setItem('gr_perfil_atual', JSON.stringify(this.perfilAtual));
                } catch(e) {}
                
                console.log('👑 Perfil Master forçado por email:', this.perfilAtual.nome);
                
                // FORÇA A ATUALIZAÇÃO GLOBAL
                window.perfilAtual = this.perfilAtual;
                if (typeof GR !== 'undefined' && GR.Modules) {
            ...
            // 2. VERIFICA SE É ADMIN PELO EMAIL
            if (user && (user.email === 'admin@gestaorural.com' || user.email === 'adm@gestaorural.com')) {
                ...
                console.log('👨‍💼 Perfil Admin forçado por email:', this.perfilAtual.nome);
                window.perfilAtual = this.perfilAtual;
                GR.Modules.Perfis.perfilAtual = this.perfilAtual;
            ...
            // 4. SE NÃO TIVER PERFIL, CRIA UM FALLBACK ADMIN
            if (!this.perfilAtual) {
                this.perfilAtual = {
                    id: 'admin',
                ...
                console.log('📋 Perfil fallback ADMIN criado com todas as permissões');
                window.perfilAtual = this.perfilAtual;
                GR.Modules.Perfis.perfilAtual = this.perfilAtual;
            }
    ...
    init: function() {
        ...
        // 🆕 FORÇA O CARREGAMENTO DO PERFIL IMEDIATAMENTE
        ...
        ref.get()
            ...
            .catch(function(err) {
                ...
                // 🆕 FORÇA A FILTRAGEM DO MENU MESMO COM ERRO
                self.filtrarMenu();
            });
    },
        ...
        return db.collection('users').doc(uid).get()
            .then(function(doc) {
                if (doc.exists) {
                    ...
                    // 🆕 VERIFICA SE O PERFIL EXISTE NO MAPA
                    if (self.perfis[perfilId]) {
                        self.perfilAtual = self.perfis[perfilId];
                    } else {
                        // Se o perfil não existe, usa operador
                        self.perfilAtual = self.perfis.operador;
                        console.warn('⚠️ Perfil "' + perfilId + '" não encontrado, usando operador');
                    }
                    ...
                    // Verifica se é o usuário master (email)
                    var user = firebase.auth().currentUser;
                    if (user && user.email === 'roqueep@gmail.com') {
                        self.perfilAtual = self.perfis.master;
                        console.log('👑 Usuário Master identificado por email');
                    } else {
                        self.perfilAtual = self.perfis.operador;
                ...
                try {
                    localStorage.setItem('gr_perfil_atual', JSON.stringify(self.perfilAtual));
                } catch(e) {}
                
                // 🆕 ATUALIZA VARIÁVEL GLOBAL
                window.perfilAtual = self.perfilAtual;
                GR.Modules.Perfis.perfilAtual = self.perfilAtual;
                
                // 🆕 FORÇA A FILTRAGEM DO MENU
                self.filtrarMenu();
    ...
    // ================================================================
    // VERIFICAR PERMISSÕES (CORRIGIDO COM FALLBACK)
    // ================================================================
    podeVer: function(modulo) {
        // 🆕 FORÇA CARREGAMENTO DO PERFIL SE NÃO TIVER
        if (!this.perfilAtual) {
            this._forcarCarregamentoPerfil();
        }
        if (!this.perfilAtual) return true; // Fallback seguro
        
        var permissoes = this.perfilAtual.permissoes;
        if (!permissoes.ver) return false;
    ...
    podeCriar: function(modulo) {
        ...
        var permissoes = this.perfilAtual.permissoes;
        if (!permissoes.criar) return false;
    ...
    podeEditar: function(modulo) {
        ...
        var permissoes = this.perfilAtual.permissoes;
        if (!permissoes.editar) return false;
    ...
    podeExcluir: function(modulo) {
        ...
        var permissoes = this.perfilAtual.permissoes;
        if (!permissoes.excluir) return false;
    ...
    podeExportar: function(modulo) {
        ...
        var permissoes = this.perfilAtual.permissoes;
        if (!permissoes.exportar) return false;
    ...
    podeImprimir: function(modulo) {
        ...
        var permissoes = this.perfilAtual.permissoes;
        if (!permissoes.imprimir) return false;
    ...
    // ================================================================
    // CARREGAR PERFIL ATUAL (FALLBACK MELHORADO)
    // ================================================================
    carregarPerfilAtual: function() {
        return this._forcarCarregamentoPerfil();
    },

    // ================================================================
    // PERMISSÕES ESPECÍFICAS DO VIVEIRO (CORRIGIDAS)
    // ================================================================
    ...
    podeAlterarPropriedadeViveiro: function() {
        if (!this.perfilAtual) {
            this._forcarCarregamentoPerfil();
        }
        if (!this.perfilAtual) return true; // Fallback seguro
        return this.perfilAtual.permissoes.viveiro_alterar_propriedade === true;
    },
    ...
    // ================================================================
    // FILTRAR MENU - CORRIGIDO COM CACHE PARA EVITAR LOOP
    // ================================================================
    filtrarMenu: function() {
        // 🆕 PREVINE EXECUÇÃO REPETIDA (debounce)
        if (this._filtroTimeout) {
            clearTimeout(this._filtroTimeout);
        }
        
        var self = this;
        this._filtroTimeout = setTimeout(function() {
            self._executarFiltroMenu();
        }, 50);
    },

    _executarFiltroMenu: function() {
        console.log('🔍 Filtrando menu por permissões...');
        
        // 🆕 FORÇA CARREGAMENTO DO PERFIL
        if (!this.perfilAtual) {
            this._forcarCarregamentoPerfil();
        }
        
        var perfil = this.perfilAtual;
        if (!perfil) {
            console.warn('⚠️ Nenhum perfil encontrado para filtrar menu!');
            return;
        }
        
        // 🆕 CACHE: Verifica se o perfil mudou
        var hashAtual = perfil.id + '_' + JSON.stringify(perfil.permissoes);
        if (this._ultimoPerfilId === hashAtual) {
            console.log('⏳ Menu já filtrado para este perfil (cache)');
            return;
        }
        this._ultimoPerfilId = hashAtual;
        
        console.log('📋 Filtrando menu para perfil:', perfil.nome);
        console.log('📋 Permissões:', perfil.permissoes);
        
        var botoes = document.querySelectorAll('.nav-btn');
        var totalVisiveis = 0;
        var isMaster = perfil.id === 'master' || perfil.id === 'admin';
        
        botoes.forEach(function(btn) {
            var section = btn.dataset.section;
            if (!section) return;
            
            // SE FOR MASTER OU ADMIN, MOSTRA TODOS OS BOTÕES
            if (isMaster) {
            ...
            // Para outros perfis, verifica permissões
            ...
            if (permissoes.ver) {
                if (permissoes.ver.includes('*')) {
                    temAcesso = true;
                } else if (permissoes.ver.includes(section)) {
                    temAcesso = true;
                }
            }
            
            btn.style.display = temAcesso ? 'flex' : 'none';
            if (temAcesso) totalVisiveis++;
        });
        
        console.log('✅ Menu filtrado:', totalVisiveis, 'botões visíveis (de', botoes.length, ')');
        
        // Atualiza também as propriedades
        this.filtrarPropriedadesUsuario();
        
        // 🆕 DISPARA EVENTO PARA OUTROS MÓDULOS
        if (window.dispatchEvent) {
            window.dispatchEvent(new CustomEvent('perfil-carregado', {
    ...
    filtrarPropriedadesUsuario: function() {
        ...
        var propriedadesPermitidas = perfil.propriedadesPermitidas;
        
        // Se for master ou não tiver restrições, mostra todas
        if (perfil.id === 'master' || perfil.id === 'admin' || !propriedadesPermitidas || propriedadesPermitidas.length === 0) {
            return;
        }
    ...
    // ================================================================
    // VERIFICAR SE É MASTER OU ADMIN (MÉTODO PÚBLICO)
    // ================================================================
    ...
    // ================================================================
    // VERIFICAR SE É MASTER (MÉTODO PÚBLICO)
    // ================================================================
    isMaster: function() {
        if (!this.perfilAtual) {
            this._forcarCarregamentoPerfil();
        ...
        if (!this.perfilAtual) return false;
        return this.perfilAtual.id === 'master';
    },
    ...
    _renderModalPerfis: function() {
        ...
        var perfis = this.perfis || this.perfisPadrao;
    ...
    _selecionarPerfil: function(perfilId) {
        ...
        // Atualiza checkboxes de permissões
        ...
        // Atualiza checkboxes de permissões especiais
    ...
    if (typeof firebase !== 'undefined' && firebase.auth) {
        firebase.auth().onAuthStateChanged(function(user) {
            ...
            if (GR.Modules && GR.Modules.Perfis) {
                ...
                // FORÇA A APLICAÇÃO DO PERFIL EM TODOS OS MÓDULOS
                setTimeout(function() {
        ...
        // Fallback: inicializa após 1 segundo
        setTimeout(function() {
...
window.GRPerfis = {
    get: function() { 
        if (GR.Modules && GR.Modules.Perfis) {
            return GR.Modules.Perfis.perfilAtual || GR.Modules.Perfis._forcarCarregamentoPerfil();
        }
        return null;
    },
    set: function(perfil) { 
        if (GR.Modules && GR.Modules.Perfis) {
            return GR.Modules.Perfis.setPerfil ? GR.Modules.Perfis.setPerfil(perfil) : null;
        }
        return null;
    },
    reload: function() { 
    ...
    list: function() { 
        if (GR.Modules && GR.Modules.Perfis) {
            return GR.Modules.Perfis.listarPerfis ? GR.Modules.Perfis.listarPerfis() : [];
        }
        return [];
    },
    check: function(perm) { 
        if (GR.Modules && GR.Modules.Perfis) {
            // Tenta todas as funções de verificação
            if (perm === 'editar' && GR.Modules.Perfis.podeEditar) {
                return GR.Modules.Perfis.podeEditar('viveiro');
            }
            if (perm === 'criar' && GR.Modules.Perfis.podeCriar) {
                return GR.Modules.Perfis.podeCriar('viveiro');
            }
            if (perm === 'excluir' && GR.Modules.Perfis.podeExcluir) {
    ...
    isMaster: function() {
        if (GR.Modules && GR.Modules.Perfis && GR.Modules.Perfis.isMaster) {
            return GR.Modules.Perfis.isMaster();
        }
        return false;
    },
    isAdmin: function() {
        if (GR.Modules && GR.Modules.Perfis && GR.Modules.Perfis.isMasterOrAdmin) {
            return GR.Modules.Perfis.isMasterOrAdmin();
        }
        return false;
    }
...
console.log('✅ Módulo Perfis v2.5 carregado com CORREÇÕES!');
console.log('📌 CORREÇÕES APLICADAS:');
console.log('   - 🔧 _forcarCarregamentoPerfil() - Força perfil Master/Admin');
console.log('   - 🔧 _temPermissao() com fallback para todas as funções');
console.log('   - 🔧 isMasterOrAdmin() - Método público para verificar');
console.log('   - 🔧 Permissões sempre retornam TRUE para Master/Admin');
console.log('   - 🔧 Fallback ADMIN com todas as permissões');
console.log('   - 🔧 Inicialização forçada mesmo sem Firebase');
console.log('   - 🔧 Cache de perfil no localStorage');
...
console.log('   - window.GRPerfis.check("editar") - Verificar permissão');
console.log('   - window.GRPerfis.isMaster() - Verificar se é Master');
console.log('   - window.GRPerfis.isAdmin() - Verificar se é Admin');