// ================================================================
// MÓDULO: VIVEIRO - SISTEMA COMPLETO COM PERMISSÕES (VERSÃO 2.9)
// ================================================================
// Versão: 2.9 - CORRIGIDO: Label "Propriedade: Carregando..." em todos os modais
// CORREÇÃO: Agora os modais mostram a propriedade correta do cabeçalho
// ================================================================
...
GR.Modules.Viveiro = {
    ...
    _atualizarLabelPropriedade: function() {
        ...
        // Lista de todos os IDs de labels de propriedade nos modais
        var labelIds = [
        ...
        // Também atualiza os selects de propriedade (se existirem e estiverem habilitados)
        var propSelects = document.querySelectorAll('[id$="-propriedade"]');
        propSelects.forEach(function(select) {
            // Só atualiza se NÃO estiver desabilitado (disabled) e não for o seletor principal
    ...
    _isMasterOrAdmin: function() {
        // 1. VERIFICA PELO SISTEMA DE PERFIS
        ...
        // 2. VERIFICA PELO EMAIL (FALLBACK)
        try {
        ...
        // 3. VERIFICA PELO PERFIL NO LOCALSTORAGE
        try {
    ...
    _temPermissao: function(permissao) {
        // 1. VERIFICA SE É MASTER PELO EMAIL (FALLBACK)
        try {
            var user = firebase.auth().currentUser;
            if (user && user.email === 'roqueep@gmail.com') {
                console.log('👑 Master - permissão concedida para:', permissao);
                return true;
            }
        } catch(e) {}
        
        // 2. VERIFICA SE É MASTER/ADMIN PELO SISTEMA DE PERFIS
        if (this._isMasterOrAdmin()) {
            console.log('👑 Master/Admin - permissão concedida para:', permissao);
            return true;
        }
        
        // 3. USA O SISTEMA DE PERFIS
        if (GR.Modules && GR.Modules.Perfis && GR.Modules.Perfis.perfilAtual) {
            ...
            // 3.1 MAPEIA PERMISSÕES DO VIVEIRO
            var mapa = {
        ...
        // 4. FALLBACK: VERIFICA NO MAPA DE PERMISSÕES PADRÃO
        ...
        // 5. PERMISSÕES BÁSICAS SEMPRE PERMITIDAS (FALLBACK SEGURO)
        var basicas = ['visualizar', 'producao'];
        if (basicas.includes(permissao)) {
            resultado = true;
        }
        
        console.log('📋 Permissão verificada:', permissao, '=', resultado);
        return resultado;
    },
    ...
    _getPerfilUsuario: function() {
        // 1. VERIFICA SE É MASTER PELO EMAIL
        try {
            var user = firebase.auth().currentUser;
            if (user && user.email === 'roqueep@gmail.com') {
    ...
    _renderInterno: function() {
        ...
        // VERIFICA SE O CONTEÚDO MUDOU (EVITA RENDER DESNECESSÁRIO)
        var hashAtual = propViveiro + '_' + (GR.State.data.viveiroMudas || []).length + '_' + (GR.State.data.viveiroVendas || []).length;
        if (this._ultimoRenderHash === hashAtual) {
            console.log('⏳ Viveiro: conteúdo não mudou, render ignorado');
            return;
        }
        this._ultimoRenderHash = hashAtual;

        // OBTÉM PERFIL DO USUÁRIO (USANDO O SISTEMA DE PERFIS)
        var perfil = this._getPerfilUsuario();
        var perfilId = perfil.id;
        
        // VERIFICA PERMISSÕES USANDO O SISTEMA DE PERFIS
        var isMasterOrAdmin = this._isMasterOrAdmin();
        var isGerente = perfilId === 'gerente';
        
        // LOG PARA DEBUG
        console.log('📋 Perfil identificado:', perfilId);
        console.log('📋 É Master/Admin?', isMasterOrAdmin);
        console.log('📋 É Gerente?', isGerente);
        
        // FORÇA PERMISSÕES - MASTER E ADMIN TÊM TUDO
        ...
        // FORÇA EDIÇÃO E CRIAÇÃO - MASTER/ADMIN/GERENTE PODEM EDITAR E CRIAR
        ...
        console.log('📋 Permissões do Viveiro:', {
            perfil: perfil.nome,
            ...
            podeCriar: podeCriar,
            podeVerProducao: podeVerProducao,
            podeVerVendas: podeVerVendas,
            podeVerCaixa: podeVerCaixa
        });
        ...
        // FILTRA OS DADOS PELA PROPRIEDADE ATIVA
        ...
        // 🆕 ATUALIZA OS LABELS DE PROPRIEDADE APÓS RENDERIZAR
        this._atualizarLabelPropriedade();
    ...
    _renderProducao: function(mudas, insumos, servicos, trabalhadores, podeCriar, podeEditar, podeExcluir) {
        ...
        // 🆕 BUSCA TODAS AS VARIEDADES PARA EXIBIR NO CARD PRODUÇÃO
        var propViveiro = this.getPropriedadeAtiva();
                        ...
                        <table style="font-size:11px;width:100%;border-collapse:collapse;">
                            ...
                            <tbody>
                                ${todasVariedades.map(function(v) {
                                    // Calcula total de mudas desta variedade
                        ...
                        <table style="font-size:11px;">
                            ...
                            <tbody>
                                ${mudas.map(function(m) {
                                    ...
                                    // BOTÃO DE EDIÇÃO - AGORA VISÍVEL QUANDO podeEditar = true
    ...
    abrirModalVenda: function(editId) {
        ...
        // 🆕 ATUALIZA O LABEL DA PROPRIEDADE
        this._atualizarLabelPropriedade();
    ...
    salvarVenda: function() {
        ...
        if (editId) {
            ...
            ref.add(dados).then(function(docRef) {
                ...
                // BAIXA O ESTOQUE DAS MUDAS (BUSCA POR VARIEDADE E STATUS PRONTA)
                if (variedade) {
                    var mudas = GR.State.data.viveiroMudas || [];
...
console.log('✅ Módulo Viveiro v2.9 carregado com CORREÇÃO DOS LABELS DE PROPRIEDADE!');
console.log('📌 CORREÇÕES APLICADAS:');
console.log('   - 🔧 CORRIGIDO: Label "Propriedade: Carregando..." em todos os modais');
console.log('   - 🔧 CORRIGIDO: Agora os modais mostram a propriedade correta do cabeçalho');
console.log('   - 🔧 CORRIGIDO: Salvamento de variedade com propriedade fixa');
console.log('   - 🔧 CORRIGIDO: Propriedade capturada corretamente do select');
console.log('   - 🔧 CORRIGIDO: Fallback para propriedade ativa se select estiver vazio');
console.log('   - 🔧 Botões de editar (✏️) aparecem para usuários com permissão');
console.log('   - 🔧 Botões de excluir (🗑️) aparecem para usuários com permissão');
console.log('   - 🔧 Botões de marcar como pronta (✅) para mudas em produção');
console.log('   - 🔧 Botões de descartar (🗑️) para mudas em produção ou prontas');
console.log('   - 🔧 Permissões forçadas para Master/Admin (editar, criar, excluir)');
console.log('   - 🔧 Gerente também pode editar e criar (mas não excluir)');
console.log('📌 FUNÇÕES DE DEBUG:');
console.log('   - console.log(GR.Modules.Viveiro._isMasterOrAdmin())');
console.log('   - console.log(GR.Modules.Viveiro._temPermissao("editar"))');
console.log('   - console.log(GR.Modules.Viveiro._getPerfilUsuario())');
console.log('   - console.log(GR.Modules.Viveiro.getPropriedadeAtiva())');