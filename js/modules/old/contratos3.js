...
GR.Modules.Contratos = {
    ...
    _buscarCliente: function(texto) {
        ...
        if (clienteMatch) {
            ...
            // Extrair apenas o nome (remover o código)
    ...
    _buscarModalidade: function(texto) {
        ...
        if (modalidadeMatch) {
            console.log('🏷️ Modalidade:', modalidadeMatch[1].trim());
            return modalidadeMatch[1].trim();
        }
        
        // 2º TENTATIVA: Modalidade com código
        var modalidadeMatch2 = texto.match(/[0-9]+-[ACI]+\s*-\s*([A-Za-zÀ-Üà-ü\s]+?)(?=\s+[0-9,])/i);
        if (modalidadeMatch2) {
            console.log('🏷️ Modalidade (padrão):', modalidadeMatch2[1].trim());
            return modalidadeMatch2[1].trim();
        }
    ...
    _buscarValorOperacao: function(texto) {
        ...
        if (valorMatch) {
            ...
            console.log('💰 Valor Operação:', parseFloat(v));
    ...
    _buscarSaldoQuitacao: function(texto) {
        ...
        if (saldoMatch) {
            ...
            console.log('💰 Saldo Quitação:', parseFloat(s));
            return parseFloat(s) || 0;
        }
        
        // 2º TENTATIVA: Saldo com padrão alternativo
        var saldoMatch2 = texto.match(/([0-9]{1,3}(?:[.][0-9]{3})*[,][0-9]{2})\s+Taxa\s+Juros/i);
        if (saldoMatch2) {
            var s = saldoMatch2[1].trim().replace(/[^0-9,]/g, '').replace(',', '.');
            console.log('💰 Saldo Quitação (alt):', parseFloat(s));
            return parseFloat(s) || 0;
        }
    ...
    _extrairTodosOsDados: function(texto) {
        ...
        // TAXAS
        var taxaJurosMatch = texto.match(/Taxa\s+Juros\s*:\s*([0-9]+[,.]?[0-9]*)\s*%/i);
        if (taxaJurosMatch) {
        ...
        if (abertoIndex !== -1) {
            ...
            // Padrão para capturar parcelas em aberto
        ...
        if (amortizacaoIndex !== -1) {
            ...
            // Padrão para capturar amortizações
            ...
            // Se não encontrou com o padrão principal, tentar padrão alternativo
            ...
            // Adicionar amortizações às parcelas liquidadas
            if (amortizacoes.length > 0) {
                // Ajustar números das parcelas para continuar a sequência
