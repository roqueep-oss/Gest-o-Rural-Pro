...
GR.Modules.Contratos = {
    ...
    // ================================================================
    // EXTRAIR TODOS OS DADOS DO TEXTO - COM MODELO 1 E MODELO 2
    // ================================================================
    _extrairTodosOsDados: function(texto) {
        ...
        // ================================================================
        // DADOS GERAIS (comuns aos 2 modelos)
        // ================================================================
        ...
        // ================================================================
        // MODELO 2: "Amortização do Saldo Devedor" (CORRIGIDO)
        // ================================================================
        else if (amortizacaoIndex !== -1) {
            console.log('📄 MODELO 2: "Amortização do Saldo Devedor" detectado!');
            
            // ============================================================
            // DADOS DO MODELO 2 (CORRIGIDO COM OS PADRÕES INDICADOS)
            // ============================================================
            
            // CONTRATO (Modelo 2) - Busca em múltiplos lugares
            ...
            if (contratoMatch2) {
                ...
                console.log('📄 Contrato (MODELO 2):', dados.contrato);
            } else {
                ...
                if (contratoAntigo) {
                    ...
                    // Buscar número no início do texto ou após "Relatório"
            ...
            // ============================================================
            // MUTUÁRIO (Modelo 2) - Busca com aspas "SEBASTIAO BEJAMIM PAGOTO"
            // ============================================================
            console.log('🔍 Buscando Mutuário (MODELO 2)...');
            
            // 1º TENTATIVA: Buscar nome entre aspas duplas
            var mutuarioMatch1 = texto.match(/"([A-Z][A-Z\s]+)"/i);
            if (mutuarioMatch1) {
                var nome = mutuarioMatch1[1].trim();
                // Verificar se parece um nome (tem pelo menos 2 palavras)
            ...
            // 3º TENTATIVA: Buscar nome com padrão "XXXXX-X NOME"
            if (!dados.cliente) {
                var clienteMatch3 = texto.match(/([0-9]+-[0-9]+\s+[A-Z][A-Z\s]+?)(?=\s+[0-9]+\s+[0-9]+-ACI)/i);
                if (clienteMatch3) {
                    var cliente = clienteMatch3[1].trim();
                    var nomeMatch = cliente.match(/[0-9]+-[0-9]+\s+(.+)/);
                    if (nomeMatch) {
                        dados.cliente = nomeMatch[1].trim();
                        console.log('👤 Mutuário (MODELO 2 - padrão):', dados.cliente);
                    }
                }
            }

            // ============================================================
            // MODALIDADE (Modelo 2) - Busca com aspas "PRONAF BNDES AGRICOLA"
            // ============================================================
            console.log('🔍 Buscando Modalidade (MODELO 2)...');
            
            // 1º TENTATIVA: Buscar modalidade entre aspas com padrão "32-PRONAF BNDES AGRICOLA"
            var modalidadeMatch1 = texto.match(/([0-9]+)-"([A-Za-zÀ-Üà-ü\s]+)"/i);
            if (modalidadeMatch1) {
                dados.modalidade = modalidadeMatch1[1] + '-' + modalidadeMatch1[2].trim();
                console.log('🏷️ Modalidade (MODELO 2 - aspas com código):', dados.modalidade);
            }
            
            // 2º TENTATIVA: Buscar modalidade entre aspas sem código
            if (!dados.modalidade) {
                var modalidadeMatch2 = texto.match(/"([A-Za-zÀ-Üà-ü\s]+)"/i);
                if (modalidadeMatch2) {
                    var modalidade = modalidadeMatch2[1].trim();
                    // Verificar se parece uma modalidade (PRONAF, BNDES, etc)
            ...
            // 3º TENTATIVA: Campo "Modalidade:"
            if (!dados.modalidade) {
                var modalidadeMatch3 = texto.match(/Modalidade\s*:\s*([0-9]+-[A-Za-zÀ-Üà-ü\s-]+)/i);
                if (modalidadeMatch3) {
                    dados.modalidade = modalidadeMatch3[1].trim();
                    console.log('🏷️ Modalidade (MODELO 2 - campo):', dados.modalidade);
                }
            }
            
            // 4º TENTATIVA: Padrão "XXXX-ACI - NOME"
            if (!dados.modalidade) {
                var modalidadeMatch4 = texto.match(/[0-9]+-ACI\s*-\s*([A-Za-zÀ-Üà-ü\s]+?)(?=\s+[0-9,])/i);
                if (modalidadeMatch4) {
                    dados.modalidade = modalidadeMatch4[1].trim();
                    console.log('🏷️ Modalidade (MODELO 2 - ACI):', dados.modalidade);
                }
            }

            // ============================================================
            // TAXA JUROS (Modelo 2) - Busca com aspas "6,0000"
            // ============================================================
            console.log('🔍 Buscando Taxa Juros (MODELO 2)...');
            
            // 1º TENTATIVA: Buscar taxa entre aspas "X,XXXX"
            ...
            if (dados.taxaJuros === 0) {
                ...
                if (taxaMatch3) {
                    ...
                    if (taxa > 0 && taxa < 100) {
                        dados.taxaJuros = taxa;
                        console.log('📊 Taxa Juros (MODELO 2 - % a.a):', dados.taxaJuros);
                    }
                }
            }

            // ============================================================
            // SALDO QUITAÇÃO (Modelo 2) - Busca com aspas "62.423,64"
            // ============================================================
            console.log('🔍 Buscando Saldo Quitação (MODELO 2)...');
            
            // 1º TENTATIVA: Buscar saldo entre aspas "X.XXX,XX"
            ...
            // 2º TENTATIVA: Campo "Saldo p/ Quitação:"
            ...
            // 3º TENTATIVA: Padrão "X.XXX,XX Taxa Juros Inad"
            if (dados.saldoQuitacao === 0) {
                var saldoMatch3 = texto.match(/([0-9]{1,3}(?:[.][0-9]{3})*[,][0-9]{2})\s+Taxa\s+Juros\s+Inad/i);
                if (saldoMatch3) {
                    var s = saldoMatch3[1].trim().replace(/[^0-9,]/g, '').replace(',', '.');
                    dados.saldoQuitacao = parseFloat(s) || 0;
                    console.log('💰 Saldo Quitação (MODELO 2 - padrão):', dados.saldoQuitacao);
                }
            }
            ...
            if (valorOperacaoMatch2) {
                ...
                dados.valorOperacao = parseFloat(v) || 0;
                console.log('💰 Valor Operação (MODELO 2):', dados.valorOperacao);
            } else {
