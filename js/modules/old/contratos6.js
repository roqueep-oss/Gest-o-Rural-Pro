...
GR.Modules.Contratos = {
    ...
    _extrairTodosOsDados: function(texto) {
        ...
        // ================================================================
            ...
            // 2º TENTATIVA: Campo "Contrato Antigo:"
            else {
                ...
                // 3º TENTATIVA: Buscar número no final da linha de dados
                else {
                    var numDoc = texto.match(/([0-9]{8})\s+[0-9]-[A-Z]/);
                    if (numDoc) {
            ...
            // ============================================================
            // MUTUÁRIO (Modelo 2) - "SEBASTIAO BEJAMIM PAGOTO"
            // ============================================================
            ...
            // 2º TENTATIVA: Buscar nome no padrão "XXXXX-X NOME"
            if (!dados.cliente) {
                var clienteMatch2 = texto.match(/([0-9]+-[0-9]+\s+[A-Z][A-Z\s]+?)(?=\s+[0-9]+)/i);
                if (clienteMatch2) {
            ...
            // ============================================================
            // MODALIDADE (Modelo 2) - "PRONAF BNDES AGRICOLA"
            // ============================================================
            console.log('🔍 Buscando Modalidade (MODELO 2)...');
            
            // 1º TENTATIVA: Buscar modalidade com padrão 32-PRONAF BNDES AGRICOLA
            var modalidadeMatch1 = texto.match(/([0-9]+)-([A-Za-zÀ-Üà-ü\s]+?)(?=\s+[0-9,])/i);
            if (modalidadeMatch1) {
                var modalidade = modalidadeMatch1[2].trim();
                if (/PRONAF|BNDES|CDC|AGRO|RURAL|CREDITO/i.test(modalidade)) {
                    dados.modalidade = modalidade;
                    console.log('🏷️ Modalidade (MODELO 2 - código):', dados.modalidade);
                }
            }
            
            // 2º TENTATIVA: Buscar modalidade entre aspas
            if (!dados.modalidade) {
            ...
            if (!dados.modalidade) {
                ...
                if (modalidadeMatch3) {
                    ...
                    var nomeModalidade = modalidade.replace(/[0-9]+-ACI\s*-\s*/, '');
                    dados.modalidade = nomeModalidade;
                    console.log('🏷️ Modalidade (MODELO 2 - campo):', dados.modalidade);
                }
            }

            // ============================================================
            // TAXA JUROS (Modelo 2) - "6,0000"
            // ============================================================
            console.log('🔍 Buscando Taxa Juros (MODELO 2)...');
            
            // 1º TENTATIVA: Buscar taxa entre aspas
            ...
            // 2º TENTATIVA: Buscar taxa no padrão "X,XXXX" após "PRONAF"
            if (dados.taxaJuros === 0) {
                var taxaMatch2 = texto.match(/PRONAF[\s\S]*?([0-9]+[,.]?[0-9]+)/i);
                if (taxaMatch2) {
                    var taxa = parseFloat(taxaMatch2[1].trim().replace(',', '.')) || 0;
                    if (taxa > 0 && taxa < 100) {
                        dados.taxaJuros = taxa;
                        console.log('📊 Taxa Juros (MODELO 2 - após PRONAF):', dados.taxaJuros);
                    }
                }
            }
            
            // 3º TENTATIVA: Campo "Taxa Juros:"
            if (dados.taxaJuros === 0) {
            ...
            // ============================================================
            // SALDO QUITAÇÃO (Modelo 2) - "62.423,64"
            // ============================================================
            console.log('🔍 Buscando Saldo Quitação (MODELO 2)...');
            
            // 1º TENTATIVA: Buscar saldo entre aspas
            ...
            // 2º TENTATIVA: Buscar saldo após "Spread da Operação:"
            if (dados.saldoQuitacao === 0) {
                var saldoMatch2 = texto.match(/Spread\s+da\s+Operação[\s\S]*?([0-9]{1,3}(?:[.][0-9]{3})*[,][0-9]{2})/i);
                if (saldoMatch2) {
                    var s = saldoMatch2[1].trim().replace(/[^0-9,]/g, '').replace(',', '.');
                    var valor = parseFloat(s) || 0;
                    if (valor > 1000) {
                        dados.saldoQuitacao = valor;
                        console.log('💰 Saldo Quitação (MODELO 2 - após Spread):', dados.saldoQuitacao);
                    }
                }
            }
            
            // 3º TENTATIVA: Campo "Saldo p/ Quitação:"
            if (dados.saldoQuitacao === 0) {
