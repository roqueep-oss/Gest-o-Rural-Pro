// ================================================================
// MÓDULO: VIVEIRO - SISTEMA COMPLETO COM PERMISSÕES (VERSÃO 2.11)
// ================================================================
// Versão: 2.11 - CORRIGIDO: UI reativa (realtime), hash de render completo, polish visual, logs limpos
// CORREÇÃO: Removidas chamadas a GR.UI.refreshCurrentView() que faziam o módulo desaparecer
// ================================================================
...
GR.Modules.Viveiro = {
    ...
    // ================================================================
    // CONTROLE DE RENDER (DEBOUNCE PARA EVITAR LOOP)
    // ================================================================
    _renderTimeout: null,
    _ultimoRenderHash: null,
    ...
    setPropriedadeAtiva: function(propriedade) {
        ...
        this._atualizarLabelPropriedade();
    ...
    // ================================================================
    // ATUALIZAR LABEL DA PROPRIEDADE EM TODOS OS MODAIS
    // ================================================================
    _atualizarLabelPropriedade: function() {
        var propViveiro = this.getPropriedadeAtiva();
        var nomeExibicao = propViveiro === 'todas' ? '🌍 Todas' : propViveiro;
        
        var labelIds = [
            'viveiro-muda-propriedade-label',
            'viveiro-insumo-propriedade-label',
            'viveiro-servico-propriedade-label',
            'viveiro-trabalhador-propriedade-label',
            'viveiro-variedade-propriedade-label',
            'viveiro-pedido-propriedade-label',
            'viveiro-venda-propriedade-label',
            'viveiro-lancamento-propriedade-label',
            'viveiro-converter-pedido-propriedade-label'
        ];
        ...
        var propSelects = document.querySelectorAll('[id$="-propriedade"]');
        propSelects.forEach(function(select) {
            if (!select.disabled && select.id !== 'viveiro-propriedade-select') {
                select.value = propViveiro;
            }
        });
        
        console.log('📍 Labels de propriedade atualizados para:', nomeExibicao);
    },
    ...
    // ================================================================
    // CONFIGURAÇÃO DE PERMISSÕES (FALLBACK)
    // ================================================================
    _permissoes: {
        'master': {
            producao: true, vendas: true, caixa: true, relatorios: true,
            exportar: true, excluir: true, editar: true, criar: true,
        ...
        'gerente': {
            producao: true, vendas: true, caixa: true, relatorios: true,
            exportar: true, excluir: false, editar: true, criar: true,
            alterar_propriedade: false
        },
        'operador': {
            producao: true, vendas: false, caixa: false, relatorios: false,
            exportar: false, excluir: false, editar: true, criar: true,
            alterar_propriedade: false
        },
        'visitante': {
            producao: true, vendas: false, caixa: false, relatorios: false,
            exportar: false, excluir: false, editar: false, criar: false,
            alterar_propriedade: false
        }
    },

    // ================================================================
    // VERIFICAR SE É MASTER OU ADMIN (USANDO PERFIS)
    // ================================================================
    _isMasterOrAdmin: function() {
        if (GR.Modules && GR.Modules.Perfis) {
            if (GR.Modules.Perfis.isMasterOrAdmin) {
        ...
        try {
            ...
            if (saved) {
                ...
                if (perfil && (perfil.id === 'master' || perfil.id === 'admin')) {
                    return true;
                }
            }
        } catch(e) {}
    ...
    // ================================================================
    // VERIFICAR PERMISSÃO - USANDO O SISTEMA DE PERFIS
    // ================================================================
    _temPermissao: function(permissao) {
        ...
        if (GR.Modules && GR.Modules.Perfis && GR.Modules.Perfis.perfilAtual) {
            ...
            var perfilId = perfil.id || 'visitante';
            ...
            if (permissao === 'editar') {
                return GR.Modules.Perfis.podeEditar('viveiro');
            }
            if (permissao === 'criar') {
                return GR.Modules.Perfis.podeCriar('viveiro');
            }
            if (permissao === 'excluir') {
                return GR.Modules.Perfis.podeExcluir('viveiro');
            }
            if (permissao === 'visualizar' || permissao === 'producao') {
                return GR.Modules.Perfis.podeVer('viveiro');
            }
            if (permissao === 'alterar_propriedade') {
                return GR.Modules.Perfis.podeAlterarPropriedadeViveiro();
            }
        }
        
        var perfilId = 'visitante';
        if (GR.Modules && GR.Modules.Perfis && GR.Modules.Perfis.perfilAtual) {
            perfilId = GR.Modules.Perfis.perfilAtual.id || 'visitante';
        }
        
        var permissoes = this._permissoes[perfilId] || this._permissoes['visitante'];
        var resultado = permissoes[permissao] === true;
        
        var basicas = ['visualizar', 'producao'];
        if (basicas.includes(permissao)) {
            resultado = true;
        }
    ...
    // ================================================================
    // OBTER PERFIL DO USUÁRIO - USANDO O SISTEMA DE PERFIS
    // ================================================================
    _getPerfilUsuario: function() {
        try {
            var user = firebase.auth().currentUser;
            if (user && user.email === 'roqueep@gmail.com') {
                return { id: 'master', nome: '👑 Master' };
            }
        } catch(e) {}
        ...
        return { id: 'visitante', nome: '👤 Visitante' };
    },

    // ================================================================
    // OBTER DADOS DO VIVEIRO (COM FILTRO DE PROPRIEDADE)
    // ================================================================
    _getDados: function() {
        var propViveiro = this.getPropriedadeAtiva();
        return {
            mudas: (GR.State.data.viveiroMudas || []).filter(function(item) {
                return propViveiro === 'todas' || item.propriedade === propViveiro;
            }),
            insumos: (GR.State.data.viveiroInsumos || []).filter(function(item) {
                return propViveiro === 'todas' || item.propriedade === propViveiro;
            }),
            servicos: (GR.State.data.viveiroServicos || []).filter(function(item) {
                return propViveiro === 'todas' || item.propriedade === propViveiro;
            }),
            trabalhadores: (GR.State.data.viveiroTrabalhadores || []).filter(function(item) {
                return propViveiro === 'todas' || item.propriedade === propViveiro;
            }),
            vendas: (GR.State.data.viveiroVendas || []).filter(function(item) {
                return propViveiro === 'todas' || item.propriedade === propViveiro;
            }),
            caixa: (GR.State.data.viveiroCaixa || []).filter(function(item) {
                return propViveiro === 'todas' || item.propriedade === propViveiro;
            }),
            pedidos: (GR.State.data.viveiroPedidos || []).filter(function(item) {
                return propViveiro === 'todas' || item.propriedade === propViveiro;
            }),
            variedades: (GR.State.data.viveiroVariedades || []).filter(function(item) {
                return propViveiro === 'todas' || item.propriedade === propViveiro;
            })
        };
    },
    ...
    // ================================================================
    // RENDER PRINCIPAL - COM DEBOUNCE PARA EVITAR LOOP
    // ================================================================
    render: function() {
        var self = this;
        // 🆕 Bind único aos eventos realtime do State (dispara re-render automático)
        ...
        this._renderTimeout = setTimeout(function() {
            self._renderInterno();
        }, 30); // debounce menor = UI mais responsiva
    },

    // 🆕 Força re-render ignorando cache de hash (usar após CRUD)
    _invalidateAndRender: function() {
        this._forceRender = true;
        this._ultimoRenderHash = null;
        this.render();
    },

    // 🆕 Assinatura leve baseada em updatedAt/id para detectar edições
    _contentSignature: function() {
        var d = GR.State.data;
        var cols = ['viveiroMudas','viveiroInsumos','viveiroServicos','viveiroTrabalhadores',
                    'viveiroVendas','viveiroCaixa','viveiroVariedades','viveiroPedidos'];
        var sig = 0;
        for (var c = 0; c < cols.length; c++) {
            var arr = d[cols[c]] || [];
            for (var i = 0; i < arr.length; i++) {
                var it = arr[i] || {};
                var t = it.dataAtualizacao || it.dataCriacao || it.data || it.id || '';
                if (typeof t === 'string') {
                    for (var k = 0; k < t.length; k++) sig = ((sig << 5) - sig + t.charCodeAt(k)) | 0;
                }
            }
        }
        return sig;
    ...
    // 🆕 Escuta os eventos disparados pelos listeners realtime do Firestore
    _bindStateEvents: function() {
        if (this._eventsBound) return;
        this._eventsBound = true;
        var self = this;
        var eventos = [
            'viveiro-mudas-atualizadas',
            'viveiro-insumos-atualizados',
            'viveiro-servicos-atualizados',
            'viveiro-trabalhadores-atualizados',
            'viveiro-vendas-atualizadas',
            'viveiro-caixa-atualizados',
            'viveiro-variedades-atualizadas',
            'viveiro-pedidos-atualizados'
        ];
        ...
        console.log('🔔 Viveiro: eventos realtime conectados (' + eventos.length + ')');
    },

    // 🆕 CSS de polish visual (injetado uma vez)
    _injectStyles: function() {
        if (document.getElementById('viveiro-polish-styles')) return;
        var css = document.createElement('style');
        css.id = 'viveiro-polish-styles';
        css.textContent = [
            '#viveiro-content .vv-card{background:var(--card,#fff);border:1px solid var(--border,#e5e7eb);border-radius:12px;padding:16px;box-shadow:0 1px 2px rgba(0,0,0,.04);transition:transform .15s ease, box-shadow .15s ease}',
            '#viveiro-content .vv-card:hover{transform:translateY(-2px);box-shadow:0 6px 18px rgba(0,0,0,.08)}',
            '#viveiro-content table{border-collapse:separate;border-spacing:0;width:100%}',
            '#viveiro-content table thead th{position:sticky;top:0;background:var(--bg,#f8fafc);z-index:1}',
            '#viveiro-content table tbody tr{transition:background .12s ease}',
            '#viveiro-content table tbody tr:hover{background:rgba(13,71,161,.05)}',
            '#viveiro-content .btn{transition:transform .1s ease, box-shadow .15s ease, opacity .15s}',
            '#viveiro-content .btn:hover{transform:translateY(-1px)}',
            '#viveiro-content .btn:active{transform:translateY(0)}',
            '#viveiro-content .vv-fade{animation:vvFade .25s ease-out}',
            '@keyframes vvFade{from{opacity:0;transform:translateY(4px)}to{opacity:1;transform:none}}',
            '#viveiro-content .vv-empty{padding:32px;text-align:center;color:var(--text-light,#64748b);border:2px dashed var(--border,#e5e7eb);border-radius:12px}',
            '#viveiro-content .badge{display:inline-block;padding:2px 8px;border-radius:999px;font-size:11px;font-weight:600;background:rgba(13,71,161,.1);color:#0d47a1}'
        ].join('\n');
        document.head.appendChild(css);
    },

    // ================================================================
    // RENDER INTERNO - LÓGICA PRINCIPAL (COMPLETA)
    // ================================================================
    _renderInterno: function() {
        var div = document.getElementById('viveiro-content');
        if (!div) return;

        var propViveiro = this.getPropriedadeAtiva();

        // 🆕 Hash inclui TODAS as coleções + timestamp de última alteração
        // para detectar edições (que não mudam length) e mudanças em qualquer coleção.
        var d = GR.State.data;
        var hashAtual = [
            propViveiro,
            (d.viveiroMudas || []).length,
            (d.viveiroInsumos || []).length,
            (d.viveiroServicos || []).length,
            (d.viveiroTrabalhadores || []).length,
            (d.viveiroVendas || []).length,
            (d.viveiroCaixa || []).length,
            (d.viveiroVariedades || []).length,
            (d.viveiroPedidos || []).length,
            this._contentSignature()
        ].join('|');
        if (!this._forceRender && this._ultimoRenderHash === hashAtual) {
            return;
        }
        this._forceRender = false;
        this._ultimoRenderHash = hashAtual;

        var perfil = this._getPerfilUsuario();
        var perfilId = perfil.id;
        
        var isMasterOrAdmin = this._isMasterOrAdmin();
        var isGerente = perfilId === 'gerente';

        var podeVerProducao = isMasterOrAdmin || this._temPermissao('producao');
        var podeVerVendas = isMasterOrAdmin || this._temPermissao('vendas');
        var podeVerCaixa = isMasterOrAdmin || this._temPermissao('caixa');
        var podeVerRelatorios = isMasterOrAdmin || this._temPermissao('relatorios');
        var podeExportar = isMasterOrAdmin || this._temPermissao('exportar');
        
        var podeCriar = isMasterOrAdmin || isGerente || this._temPermissao('criar');
        var podeEditar = isMasterOrAdmin || isGerente || this._temPermissao('editar');
        var podeExcluir = isMasterOrAdmin || this._temPermissao('excluir');
        var podeAlterarProp = isMasterOrAdmin || this._temPermissao('alterar_propriedade');
                    ...
                    <div style="padding:12px 16px;">
                        ...
                        <div style="margin-top:10px;border-top:1px solid var(--border-light);padding-top:8px;">
                            <div style="display:flex;justify-content:space-between;font-size:10px;color:var(--text-light);margin-bottom:4px;">
                                ...
                                <button class="btn btn-link btn-sm" onclick="GR.Modules.Viveiro._mostrarSubAba('viveiro-producao')" style="font-size:9px;padding:0;color:#1565C0;font-weight:700;text-decoration:underline;cursor:pointer;background:transparent;border:none;">
                                    Ver todas →
                                </button>
                            </div>
                            <div style="max-height:80px;overflow-y:auto;font-size:10px;">
                                ${mudas.slice(0, 3).map(function(m) {
                                    var statusIcon = m.status === 'Pronta' ? '✅' : m.status === 'Produção' ? '⏳' : m.status === 'Descartada' ? '🗑️' : '📋';
                                    return `<div style="display:flex;justify-content:space-between;padding:2px 0;border-bottom:1px solid var(--border-light);">
                                        <span>${statusIcon} ${GR.Utils.escapeHtml(m.especie)} ${m.variedade ? '- ' + GR.Utils.escapeHtml(m.variedade) : ''}</span>
                ...
                <div class="card" style="padding:0;overflow:hidden;border-top:4px solid #2196F3;">
                    ...
                    <div style="padding:12px 16px;">
                        ...
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:11px;">
                            ...
                            <!-- 🔧 REMOVIDO: Bloco de "Variedades" do Card Comercial -->
                        </div>
                        <div style="margin-top:10px;border-top:1px solid var(--border-light);padding-top:8px;">
                            <div style="display:flex;justify-content:space-between;font-size:10px;color:var(--text-light);margin-bottom:4px;">
                                <span>📋 Últimas vendas</span>
                                <button class="btn btn-link btn-sm" onclick="GR.Modules.Viveiro._mostrarSubAba('viveiro-vendas')" style="font-size:9px;padding:0;color:#1565C0;font-weight:700;text-decoration:underline;cursor:pointer;background:transparent;border:none;">
                                    Ver todas →
                                </button>
                            </div>
                            <div style="max-height:80px;overflow-y:auto;font-size:10px;">
                ...
                <div style="padding:12px 16px;">
                    ...
                    <div style="border-top:1px solid var(--border-light);padding-top:10px;">
                        <div style="display:flex;justify-content:space-between;font-size:10px;color:var(--text-light);margin-bottom:4px;">
                            ...
                            <button class="btn btn-link btn-sm" onclick="GR.Modules.Viveiro._mostrarSubAba('viveiro-caixa')" style="font-size:9px;padding:0;color:#1565C0;font-weight:700;text-decoration:underline;cursor:pointer;background:transparent;border:none;">
                                Ver todos →
                            </button>
                        </div>
        ...
        html += `
            <div style="margin-top:0;">
                ...
                <div id="viveiro-vendas" class="sub-aba-content" style="display:none;">
                    ${podeVerVendas ? this._renderVendas(vendas, pedidos, podeCriar, podeEditar, podeExcluir) : this._renderAcessoNegado('Vendas')}
                </div>
        ...
        console.log('📋 Permissões finais - Editar:', podeEditar, 'Excluir:', podeExcluir, 'Criar:', podeCriar);
    },
    ...
    // ================================================================
    // MARCAR MUDA INDIVIDUAL COMO DESCARTADA
    // ================================================================
    marcarMudaDescartada: function(id) {
        if (!id) {
            GR.Toast.error('ID da muda não informado!');
            return;
        }

        if (!confirm('Descartar esta muda?')) return;
        ...
        db.collection('users').doc(uid).collection('viveiroMudas').doc(id).update({
            status: 'Descartada',
            dataDescartes: new Date().toISOString().split('T')[0]
        }).then(function() {
            GR.Toast.success('🗑️ Muda descartada!');
            GR.Modules.Viveiro._invalidateAndRender();
        }).catch(function(err) {
            GR.Toast.error('Erro ao descartar: ' + err.message);
        });
    },
    ...
    // ================================================================
    // RENDER PRODUÇÃO - COM BOTÕES PRONTA E DESCARTAR
    // ================================================================
    _renderProducao: function(mudas, insumos, servicos, trabalhadores, podeCriar, podeEditar, podeExcluir) {
        var totalMudasQtd = mudas.reduce(function(sum, m) { return sum + (m.quantidade || 0); }, 0);

        var propViveiro = this.getPropriedadeAtiva();
        var todasVariedades = GR.State.data.viveiroVariedades || [];
        
        if (propViveiro !== 'todas') {
            todasVariedades = todasVariedades.filter(function(v) { 
                return v.propriedade === propViveiro; 
            });
        }
        ...
        return `
            ...
            <!-- CARD DE VARIEDADES (AGORA NO PRODUÇÃO) -->
            <div class="card" style="padding:12px;margin-bottom:10px;border-left:4px solid #9C27B0;">
                <div class="card-header" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:6px;margin-bottom:8px;">
                    <div class="card-title" style="font-size:14px;font-weight:700;">🌱 Variedades Cadastradas</div>
                    <div style="display:flex;gap:4px;flex-wrap:wrap;">
                        ${podeCriar ? `<button class="btn btn-sm btn-success" onclick="GR.Modules.Viveiro.abrirModalVariedade()" style="font-size:10px;padding:4px 10px;">➕ Nova Variedade</button>` : ''}
                    </div>
                </div>
                
                ${!todasVariedades.length ? `
                    <div class="empty-state" style="padding:20px;text-align:center;">
                        <span class="icon" style="font-size:32px;">🌱</span>
                        <div class="message" style="font-size:13px;color:var(--text-light);">Nenhuma variedade cadastrada</div>
                    </div>
                ` : `
                        ...
                        <table style="font-size:11px;width:100%;border-collapse:collapse;">
                            <thead>
                                <tr style="background:var(--bg);">
                                    <th style="padding:6px 8px;text-align:left;border-bottom:2px solid var(--border);">Nome</th>
                                    <th style="padding:6px 8px;text-align:left;border-bottom:2px solid var(--border);">Espécie</th>
                                    <th style="padding:6px 8px;text-align:center;border-bottom:2px solid var(--border);">Preço</th>
                                    <th style="padding:6px 8px;text-align:center;border-bottom:2px solid var(--border);">Estoque Mín</th>
                                    <th style="padding:6px 8px;text-align:center;border-bottom:2px solid var(--border);">Tempo Prod.</th>
                                    <th style="padding:6px 8px;text-align:center;border-bottom:2px solid var(--border);">Ações</th>
                                </tr>
                            </thead>
                            <tbody>
                                ${todasVariedades.map(function(v) {
                                    var totalMudasVariedade = mudas.filter(function(m) {
                                        return m.variedade === v.nome;
                                    }).reduce(function(sum, m) {
                                        return sum + (m.quantidade || 0);
                                    }, 0);
                                    
                                    return `
                                        <tr style="border-bottom:1px solid var(--border-light);">
                                            <td style="padding:6px 8px;font-weight:600;">${GR.Utils.escapeHtml(v.nome)}</td>
                                            <td style="padding:6px 8px;">${GR.Utils.escapeHtml(v.especie || '-')}</td>
                                            <td style="padding:6px 8px;text-align:center;">${GR.Utils.formatarMoedaBR(v.preco || 0)}</td>
                    ...
                    <div style="margin-top:8px;font-size:10px;color:var(--text-light);text-align:right;">
                        Total: <strong>${todasVariedades.length}</strong> variedades cadastradas
                    </div>
                `}
            </div>

            <!-- LISTA DE MUDAS COM BOTÕES PRONTA E DESCARTAR -->
    ...
    // ================================================================
    // RENDER PEDIDOS (SEPARADO - CHAMADO PELO _renderVendas)
    // ================================================================
    _renderPedidos: function(pedidos, podeCriar, podeEditar, podeExcluir) {
        if (!pedidos || pedidos.length === 0) {
            return `
                <div class="card" style="padding:12px;margin-bottom:10px;">
                    <div class="card-header" style="margin-bottom:8px;flex-wrap:wrap;gap:6px;">
                        <div class="card-title" style="font-size:14px;">📋 Pedidos</div>
                    ...
                    <div class="empty-state" style="padding:20px;">
                        <span class="icon" style="font-size:32px;">📋</span>
                        <div class="message" style="font-size:13px;">Nenhum pedido cadastrado</div>
                    </div>
                </div>
            `;
        }
        ...
        return `
            <div class="card" style="padding:12px;margin-bottom:10px;">
                <div class="card-header" style="margin-bottom:8px;flex-wrap:wrap;gap:6px;">
                    ...
                    <div style="display:flex;gap:4px;flex-wrap:wrap;">
                        ${podeCriar ? `<button class="btn btn-success btn-sm" onclick="GR.Modules.Viveiro.abrirModalPedido()" style="font-size:10px;">➕ Novo Pedido</button>` : ''}
                        ${pedidosPendentes.length > 0 ? `<span style="font-size:10px;background:#FF9800;color:#fff;padding:2px 8px;border-radius:10px;">${pedidosPendentes.length} pendentes</span>` : ''}
                    </div>
                </div>
                <div class="table-responsive">
                    <table style="font-size:11px;">
                        ...
                        <tbody>
                            ${pedidos.map(function(p) {
                                var statusColors = {
                                    'Pendente': '#FF9800',
                                    'Confirmado': '#2196F3',
                                    'Pronto para Entrega': '#4CAF50',
                                    'Entregue': '#9E9E9E',
                                    'Cancelado': '#f44336'
                                };
                                var statusColor = statusColors[p.status] || '#9E9E9E';
                                return `
                                    <tr>
                                        <td><strong>${GR.Utils.escapeHtml(p.cliente || 'N/A')}</strong></td>
                                        <td>${GR.Utils.escapeHtml(p.variedade || 'N/A')}</td>
    ...
    // ================================================================
    // RENDER VENDAS - COM EDIÇÃO E PEDIDOS (CHAMA _renderPedidos)
    // ================================================================
    _renderVendas: function(vendas, pedidos, podeCriar, podeEditar, podeExcluir) {
        var totalVendasQtd = vendas.reduce(function(sum, v) { return sum + (v.quantidade || 0); }, 0);
        var totalReceitas = vendas.reduce(function(sum, v) { return sum + (v.valorTotal || 0); }, 0);
        var pedidosPendentes = pedidos.filter(function(p) { return p.status === 'Pendente' || p.status === 'Confirmado'; });
        ...
        return `
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:10px;margin-bottom:12px;">
                ...
                <div class="card" style="padding:12px;text-align:center;border-top:3px solid #FF9800;">
                    <div style="font-size:22px;font-weight:700;color:#FF9800;">${pedidosPendentes.length}</div>
                    <div style="font-size:11px;color:var(--text-light);">📋 Pedidos Pendentes</div>
                </div>
            </div>

            ${this._renderPedidos(pedidos, podeCriar, podeEditar, podeExcluir)}

            <!-- LISTA DE VENDAS -->
            <div class="card" style="padding:12px;">
                <div class="card-header" style="margin-bottom:8px;flex-wrap:wrap;gap:6px;">
                    <div class="card-title" style="font-size:14px;">💰 Vendas Realizadas</div>
                    <div style="display:flex;gap:4px;flex-wrap:wrap;">
    ...
    salvarVariedade: function() {
        ...
        var propriedade = propSelect ? propSelect.value : null;
        if (!propriedade || propriedade === '' || propriedade === 'Selecione...') {
            propriedade = this.getPropriedadeAtiva();
        }
        if (!propriedade || propriedade === '' || propriedade === 'todas') {
            propriedade = 'Padrão';
            console.warn('⚠️ Propriedade não definida, usando "Padrão" como fallback');
        }

        console.log('📝 Salvando variedade com propriedade:', propriedade);
        ...
        if (editId) {
            ref.doc(editId).update(dados)
                ...
                .catch(function(err) {
                    console.error('❌ Erro ao atualizar variedade:', err);
                    GR.Toast.error('Erro ao atualizar: ' + err.message);
                });
        } else {
            ref.add(dados)
                ...
                .catch(function(err) {
                    console.error('❌ Erro ao salvar variedade:', err);
                    GR.Toast.error('Erro ao salvar: ' + err.message);
                });
        }
    },
...
console.log('✅ Módulo Viveiro v2.10 carregado com CORREÇÃO!');
console.log('📌 CORREÇÕES APLICADAS:');
console.log('   - 🔧 REMOVIDO: "Variedades" do Card Comercial');
console.log('   - 🔧 REMOVIDO: GR.UI.refreshCurrentView() de TODAS as funções de salvamento');
console.log('   - ✅ Módulo NÃO desaparece mais ao salvar');
console.log('   - ✅ Variedades agora aparecem APENAS no Card Produção e Dashboard');
console.log('   - ✅ Card Comercial mostra apenas: Vendas, Receita Total e Pedidos');