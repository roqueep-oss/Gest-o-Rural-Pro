...
GR.UI = {
    ...
    _atualizarDashboard: function() {
        ...
        try {
            ...
            this._dadosChartReceitasPorMes = {};
            this._dadosChartDespesasPorMes = {};
            receitas.forEach(function(r) {
                if (!r.data) return;
                var key = new Date(r.data).getMonth() + '-' + new Date(r.data).getFullYear();
                this._dadosChartReceitasPorMes[key] = (this._dadosChartReceitasPorMes[key] || 0) + (r.valor || 0);
            }.bind(this));
            despesas.forEach(function(d) {
                if (!d.data) return;
                var key = new Date(d.data).getMonth() + '-' + new Date(d.data).getFullYear();
                this._dadosChartDespesasPorMes[key] = (this._dadosChartDespesasPorMes[key] || 0) + (d.valor || 0);
            }.bind(this));

            this._renderChartFinanceiro();
            this._renderAcoesRecentes();
            this._renderAlertas();
    ...
    _atualizarTendencias: function() {
        ...
        var el = this._getElement('dashTendencia');
        if (!el) return;
    ...
    _renderDashboard: function(container) {
        ...
        if (!container) container = this._getElement('sectionContainer');
        if (!container) return;
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;">
                        ...
                        <button class="btn btn-info btn-sm" onclick="GR.UI._atualizarDashboard()" title="Atualizar dados">🔄 Atualizar</button>
                        <button class="btn btn-secondary btn-sm" onclick="GR.UI._exportarDashboard()" title="Exportar dados">📤 Exportar</button>
                    </div>
                </div>
                    ...
                    <div style="background:var(--card-bg,#f8f9fa);border-radius:8px;padding:12px;margin:12px 0;border:1px solid var(--border,#ddd);">
                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;flex-wrap:wrap;gap:4px;">
                            ...
                            <div style="display:flex;gap:4px;">
                                <button class="btn btn-sm btn-outline-primary" onclick="GR.UI._alterarTipoGrafico('bar')" id="btn-grafico-bar" style="padding:2px 8px;font-size:10px;border:1px solid var(--primary);background:var(--primary);color:#fff;border-radius:3px;cursor:pointer;">📊 Barras</button>
                                <button class="btn btn-sm btn-outline-primary" onclick="GR.UI._alterarTipoGrafico('line')" id="btn-grafico-line" style="padding:2px 8px;font-size:10px;border:1px solid var(--border);background:transparent;color:var(--text-light);border-radius:3px;cursor:pointer;">📈 Linha</button>
                            </div>
                        </div>
    ...
    _alterarTipoGrafico: function(tipo) {
        this._tipoGraficoDashboard = tipo;
    ...
    _renderChartFinanceiro: function() {
        ...
        if (!canvas) return;
        if (this._chartFinanceiro) { this._chartFinanceiro.destroy(); this._chartFinanceiro = null; }

        var tipo = this._tipoGraficoDashboard || 'bar';
        ...
        for (var i = 11; i >= 0; i--) {
            ...
            receitasData.push(this._dadosChartReceitasPorMes ? (this._dadosChartReceitasPorMes[d.getMonth() + '-' + d.getFullYear()] || 0) : 0);
            despesasData.push(this._dadosChartDespesasPorMes ? (this._dadosChartDespesasPorMes[d.getMonth() + '-' + d.getFullYear()] || 0) : 0);
        }
