...
GR.UI = {
    ...
    refreshCurrentView: function() {
        ...
        var renderMap = {
            'dashboard': this._renderDashboard.bind(this),
    ...
    // ================================================================
    // ATUALIZAR DASHBOARD - COM FILTRO DE PROPRIEDADE
    // ================================================================
    _atualizarDashboard: function() {
        console.log('📊 Atualizando dashboard com filtro de propriedade...');
        ...
        try {
            ...
            var dashTarefas = this._getElement('dashTarefas');
            if (dashTarefas) {
                dashTarefas.textContent = tarefasFiltradas.length;
            }
            
            var dashContratos = this._getElement('dashContratos');
            if (dashContratos) {
                dashContratos.textContent = contratosFiltrados.length;
            }
            
            var dashInsumos = this._getElement('dashInsumos');
            if (dashInsumos) {
                dashInsumos.textContent = insumosFiltrados.length;
            }
            
            var dashAnimais = this._getElement('dashAnimais');
            if (dashAnimais) {
                dashAnimais.textContent = animaisFiltrados.length;
            }
            
            var dashFuncionarios = this._getElement('dashFuncionarios');
            if (dashFuncionarios) {
                dashFuncionarios.textContent = funcionariosFiltrados.length;
            }

            var totalR = receitasFiltradas.reduce(function(s, r) { return s + (r.valor || 0); }, 0);
            var totalD = despesasFiltradas.reduce(function(s, d) { return s + (d.valor || 0); }, 0);
            
            var dashSaldo = this._getElement('dashSaldo');
            if (dashSaldo) {
                dashSaldo.textContent = GR.Utils.formatarMoedaBR(totalR - totalD);
            }

            this._atualizarTendencias();

            console.log('✅ Dashboard atualizado!');
            console.log('📊 Dados filtrados:', {
                tarefas: tarefasFiltradas.length,
                contratos: contratosFiltrados.length,
                insumos: insumosFiltrados.length,
                animais: animaisFiltrados.length,
                funcionarios: funcionariosFiltrados.length
            });
        } catch (e) {
    ...
    _atualizarTendencias: function() {
        ...
        var receitasMes = receitasFiltradas.filter(function(r) {
            if (!r.data) return false;
            var data = new Date(r.data);
            return data.getMonth() === mesAtual && data.getFullYear() === anoAtual;
        });
        
        var totalReceitasMes = receitasMes.reduce(function(s, r) { return s + (r.valor || 0); }, 0);
        
        var despesasMes = despesasFiltradas.filter(function(d) {
            if (!d.data) return false;
            var data = new Date(d.data);
            return data.getMonth() === mesAtual && data.getFullYear() === anoAtual;
        });
        
        var totalDespesasMes = despesasMes.reduce(function(s, d) { return s + (d.valor || 0); }, 0);
        var saldoMes = totalReceitasMes - totalDespesasMes;
        
        var tendenciaEl = this._getElement('dashTendencia');
        if (tendenciaEl) {
            if (saldoMes > 0) {
                tendenciaEl.innerHTML = `📈 Saldo positivo este mês: ${GR.Utils.formatarMoedaBR(saldoMes)}`;
                tendenciaEl.style.color = 'var(--success)';
            } else if (saldoMes < 0) {
                tendenciaEl.innerHTML = `📉 Saldo negativo este mês: ${GR.Utils.formatarMoedaBR(saldoMes)}`;
                tendenciaEl.style.color = 'var(--danger)';
            } else {
                tendenciaEl.innerHTML = `📊 Saldo equilibrado este mês`;
                tendenciaEl.style.color = 'var(--text-light)';
            }
        }
    },
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;">
                        ...
                        <button class="btn btn-info btn-sm" onclick="GR.UI._atualizarDashboard()" title="Atualizar dados do dashboard">🔄 Atualizar</button>
                        <button class="btn btn-secondary btn-sm" onclick="GR.UI._exportarDashboard()" title="Exportar dados do dashboard">📤 Exportar</button>
                    </div>
                </div>
                <div id="dashboardContent">
                    <div class="stats-grid" id="dashboardStats">
                        ...
                        <div class="stats-card"><div class="number" id="dashSaldo">R$ 0,00</div><div class="label">💰 Saldo</div></div>
                    </div>
                    <div id="dashTendencia" style="text-align:center;padding:8px;font-size:14px;font-weight:600;margin-top:8px;"></div>
                </div>
            </div>
        `;
    ...
    _exportarDashboard: function() {
        try {
            ...
            GR.Toast.success('✅ Dashboard exportado com sucesso!');
        } catch (error) {
    ...
    _renderPecuaria: function(container) {
        ...
        `;
        if (GR.Modules.Pecuaria && typeof GR.Modules.Pecuaria.renderAnimais === 'function') GR.Modules.Pecuaria.renderAnimais();
    },
